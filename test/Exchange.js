    ...
    describe('Order actions', async () => {
        ...
        describe('Filling orders', async () => {
            ...
            describe('Success', async () => {
                ...
                it('executes the trades and charge fees', async () => {
                ...
                it('emits a Trade event', async () => {
                    ...
                    const args = event.args;
                    expect(args.id).to.equal(1, 'id is correct');
                    expect(args.user).to.equal(user2.address, 'user address is correct');
                    expect(args.tokenGet).to.equal(token2.address, 'tokenGet is correct');
                    expect(args.amountGet).to.equal(tokens(1), 'amountGet is correct');
                    expect(args.tokenGive).to.equal(token1.address, 'tokenGive is correct');
                    expect(args.amountGive).to.equal(tokens(1), 'amountGive is correct');
                    expect(args.creator).to.equal(user1.address, 'creator is correct');
                    expect(args.timestamp).to.at.least(1, 'timestamp is present');
                });
            });
